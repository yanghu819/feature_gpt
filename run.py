...
class WikiDataset(Dataset):
    def __init__(self, config):
        ...
        # 将文本分割成较小的块
        chunk_size = 10000  # 每个块的大概字符数
        self.text_chunks = [
            clean_text(text[i:i+chunk_size])
...
class Block(nn.Module):
    ...
    def forward(self, x, time_emb, v_first=None):
        B = time_emb.size(0)
        time_emb = time_emb.view(B, 4, -1)
        scale1, shift1, scale2, shift2 = time_emb.unbind(dim=1)
        
        attn_out = self.attn(self.adaln1(x, scale1, shift1), v_first=v_first)
        x = x + attn_out[0]  # 只使用第一个返回值
    ...
    with open(f'log_{use_randn}.txt', 'a+') as f:
        ...
        if use_randn == 'True':
            ...
            with torch.no_grad():
                ...
                for time in t:
                    pred_noise = model(x, time.unsqueeze(0)) / scale_factor
